...
The user-facing privacy statement now lives at **[`/privacy.html`](../privacy.html)**
so it is reachable from the deployed app itself (footer → "Privacy & Data"),
not only from this repository.

**Edit `privacy.html`, not this file** — this stub exists only so repository
browsers can find the statement. Summary of the current policy:

- Local-first: all assessment data lives in the browser's `localStorage` only.
