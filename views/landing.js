...
// (redesign R2; docs/prototype/redesign/proto.js landingHTML and its mounts).
//
// Top to bottom: the hero (the gilt star over LIFE BALANCE INDEX, which
// follows the pointer on a laptop and bursts when it lets go or is tapped), WHY
// with its typed headline, HOW IT WORKS, the eight regions as cards that slide
// in as they come up the screen, a photo band whose pictures wipe over each
// other on scroll, and the call to begin.
//
// Everything here is decoration around three paragraphs and one link. The
// markup is the finished page; reduced motion leaves it exactly so, and every
// piece of motion ends with the mount (views/motion-mount.js), which the next
// route disposes.
...
import { typedMarkup, typeIn, burst, onAbort, SPRITES } from "./stage.js";
...
// star and the parts spring home.
const HERO = {
...
const label = (text) => `<h2 class="label">(${escapeHtml(text)})</h2>`;
const EVERY_MOTIF = CHAPTERS.map(c => ({ motif: c.aspect, hue: c.hue }));

function heroMarkup() {
  return `
    <section class="hero">
      <div class="hero-stage">
        <h2 class="sr-only">Life Balance Index</h2>
        <div class="burst-layer" aria-hidden="true"></div>
        <div class="lockup" aria-hidden="true">
          <div class="part part-mark" data-part="mark"><div class="mark"><svg viewBox="0 0 100 100"><use href="${SPRITES}#star"/></svg></div></div>
          <div class="part" data-part="word"><div class="word">LIFE BALANCE</div></div>
          <div class="part" data-part="inc"><div class="inc">INDEX</div></div>
        </div>
        <button class="mark-hit" type="button" aria-label="${escapeHtml(t("Play with the star"))}"></button>
      </div>
    </section>`;
}
...
function bandMarkup() {
  return `
    <section class="photoband" aria-hidden="true">
      ${BAND_REGIONS.map(i => `<i><b style="background-image: url('./assets/regions/${CHAPTERS[i].art}.jpg');"><span>${escapeHtml(CHAPTERS[i].region)}</span></b></i>`).join("")}
    </section>`;
}
  ...
  return `
    <div class="landing">
      ...
      <section class="panel mission"><div class="wrap split">
        ${label(t("Why"))}
        <p class="headline mission-head">${typedMarkup([t("Eight parts of one life,"), t("measured against the evidence.")])}</p>
      </div></section>
...
// --- the hero ------------------------------------------------------------
//
// Three parts (the star, the word, INDEX) chase a target set by the pointer,
// the whole lockup turning and growing with the pointer's product offset. Past
// the field, or on leaving the stage, the star bursts and everything springs
// home. On a phone there is no pointer to follow, so a tap on the star does the
// burst and a knock that springs back.
function mountHero(root, scope) {
  ...
  const run = () => {
    ...
    loop({ step, signal: scope.signal, reduced: "end" })
      .catch(err => console.error("Landing hero motion failed:", err))
  ...
  const fire = () => burst(layer, markEl, { motifs: EVERY_MOTIF, signal: scope.signal })
    .catch(err => console.error("Landing burst failed:", err));
...
// --- WHY: the headline types itself once it is well into view --------------
function mountMission(root, scope) {
  ...
  const io = new IntersectionObserver((entries) => {
    ...
    typeIn(typed, { msPerChar: TYPE_MS_PER_CHAR, signal: scope.signal })
      .catch(err => console.error("Landing headline failed:", err));
  }, { threshold: 0.6 });
...
function mountScroll(root, scope) {
  ...
  const frame = () => {
    ...
    running = true;
    loop({ step, signal: scope.signal, reduced: "end" })
      .catch(err => console.error("Landing scroll motion failed:", err))
